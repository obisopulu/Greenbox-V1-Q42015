...
<?php 
if ($_POST['activityName'] == '' or $_POST['activityInfo'] == '' or $_POST['activityDate'] == '') 
	{?>
		<form method="POST" action="alteractivityprocessor.php">
		<table align="center" bgcolor="#444"><tr>
		<td style="padding:5px; font-size:12px;">Activity Name(50)<br />
		<div  style="float:left;"><input size="10" required type='text' name='activityName' maxlength='50' value='' style="color:#444; background-color:#CCC; border:none; padding:5px"></div>
		</td><td style="padding:5px; font-size:12px;">Activity Info(50)<br/>
		<div  style="float:left;"><input size="10" required type='text' name='activityInfo' maxlength='50' value='' style="color:#444; background-color:#CCC; border:none; padding:5px"></div>
		</td><td style="padding:5px; font-size:12px;">Activity Date(09(M)/15(D)/2015(Y))<br/>
		<div  style="float:left;"><input size="10" type='text' name='activityDate' maxlength='50' value='' style="color:#444; background-color:#CCC; border:none; padding:5px"></div>
	...
	while($row = mysql_fetch_array($result))
		{
            ...
            <form action="badelete.php" method="post">
            ...
            <form action="baedit.php" method="post">
	        <td style='padding:5px 5p; background-color:#337;'><input type='hidden' name='theID' value='<?php echo $activityID ?>'/><input type="submit" name="edit" value="Edit" style="margin-left:20px; cursor:pointer; border:none; background-color:#55d; color:#CCC; padding:10px"/><input type="submit" name="" value="" style="margin-left:20px; cursor:pointer; border:none; background-color:#D55; color:#CCC; padding:10px"/></td>
            </form>
	        </tr>
	        <?php $r++; } ?>
	        </table>
	...
	else
		{
			...
			echo"<form method='POST' action='alteractivityprocessor.php'>
		<table align='center' bgcolor='#444'><tr>
		<td style='padding:5px; font-size:12px;'>activity Name(50)<br />
