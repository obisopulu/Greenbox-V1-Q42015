...
<meta name="keywords" content="nigeria,music,label,activities,song,industry,greenbox" />
...
<tr>
	<td align='center'>
<input type='button' value='' style='background-image:url(img/favi.ico); background-size:contain; background-position:center; background-repeat:no-repeat; border:none; height:100px; width:100px; cursor:none; background-color:#EEE'/>		
		</form>
	</td>
</tr>
<tr>
...
echo "<table align='center' width='150px' height='150px' style='float:left; background:url(labelsimg/$labelPic) no-repeat; background-size:contain;  -webkit-filter: grayscale(100%); margin:1px;'>
		<tr><td align='right'  style='text-shadow:1px 1px 1px #000'>
		<h2>";
		if ($dateDay <10) {echo "0$dateDay";}else {echo $dateDay;} echo"<span style='color:#FFF;'>"; if ($dateMonth <10) {echo "0$dateMonth";}else {echo $dateMonth;} echo"&nbsp;</span>
		</td></tr><tr><td></td></tr>
...
while($row=mysql_fetch_assoc($result3))
	...
	{extract($row);
		...
		<tr><td align='left'  style='text-shadow:1px 1px 1px #000'><span>"; echo substr_replace($activityInfo,'...',80); echo "<br/><a href='activities.php#{$row['activityDay']}{$row['activityMonth']}{$row['activityYear']}' style='display:compact; color:#1f7044;'>more</span></a></td></tr></table>";
	}
